plot_parm_dist <- function(subset, parm, title, xlab, file, true_value){
  ggplot(subset, aes(x=x, color=method, fill=method)) + geom_density(alpha=.2) +
    labs(title = title, subtitle = paste("True value =", true_value)) +
    ylab("Density") + xlab(xlab) + 
    scale_color_manual(name="Calculation Method:", labels=c("Pseudo-likelihood", "Quadrature"),
                       values = c("#249ab5", "#a5228d")) +
    scale_fill_manual(name="Calculation Method:", labels=c("Pseudo-likelihood", "Quadrature"),
                      values = c("#249ab5", "#a5228d")) +
    geom_vline(xintercept=true_value) + theme(legend.position = c(0.85, 0.85),
